		...
		try {
			...
			if (m.matches()) {
				...
				while (!auxParamString.equals("")) {
					...
					{
						param = m.group(4); // string array without the curly
											// brackets
						Pattern parr = Pattern
								.compile("^\\s*\"((?:[^\\\\\"]|\\\\.)*)\"\\s*(?:,|$)(.*)$");
						Matcher marr;
						...
						types.add(String[].class);
					} else if (param.equals("TRUE") || param.equals("FALSE")) // it
																				// is
																				// a
																				// boolean
					{
						...
						types.add(Boolean.TYPE);
														...
														// or int[] null
					{
						Pattern pnull = Pattern
								.compile("^\\(\\s*(String|int)\\s*(\\[\\s*\\])?\\s*\\)\\s*null\\s*$");
					...
					{
						param = m.group(2); // int array without the square
											// brackets
						Pattern parr = Pattern
								.compile("^\\s*(\\d+)\\s*(?:,|$)(.*)$");
						Matcher marr;
